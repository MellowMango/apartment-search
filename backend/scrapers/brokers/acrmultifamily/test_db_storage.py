#!/usr/bin/env python
"""
Test script for database storage of ACR Multifamily properties.
...
from backend.scrapers.brokers.acrmultifamily.test_acrmultifamily_scraper import test_acr_scraper

async def test_database_storage():
    """
    Test storing ACR Multifamily property data in both Supabase and Neo4j databases.
    """
    logger.info("Testing database storage for ACR Multifamily properties")
    
    # First, extract property data using the existing test script
    logger.info("Extracting properties using the ACR Multifamily scraper")
    extracted_data = await test_acr_scraper()
    ...
    storage = DataStorage("acrmultifamily")
