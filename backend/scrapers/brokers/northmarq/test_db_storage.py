#!/usr/bin/env python
"""
Test script for database storage of northmarq properties.
...
from backend.scrapers.brokers.northmarq.test_northmarq_scraper import test_northmarq_scraper

async def test_database_storage():
    """
    Test storing northmarq property data in both Supabase and Neo4j databases.
    """
    logger.info("Testing database storage for northmarq properties")
    
    # First, extract property data using the existing test script
    logger.info("Extracting properties using the northmarq scraper")
    extracted_data = await test_northmarq_scraper()
    ...
    storage = ScraperDataStorage("northmarq", save_to_db=True)
