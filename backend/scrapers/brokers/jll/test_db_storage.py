#!/usr/bin/env python
"""
Test script for database storage of JLL properties.
...
from backend.scrapers.brokers.jll.test_jll_scraper import test_jll_scraper

async def test_database_storage():
    """
    Test storing JLL property data in both Supabase and Neo4j databases.
    """
    logger.info("Testing database storage for JLL properties")
    
    # First, extract property data using the existing test script
    logger.info("Extracting properties using the JLL scraper")
    extracted_data = await test_jll_scraper()
    ...
    storage = ScraperDataStorage("jll", save_to_db=True)
