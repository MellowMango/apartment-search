#!/usr/bin/env python
"""
Test script for MCP-based scraping of JLL's multifamily properties.
This script demonstrates property listing extraction from JLL's website.
"""
...
async def test_jll_scraper() -> Dict[str, Any]:
    """
    Test the JLL scraper.
    
    Returns:
        Extracted property data
    """
    from backend.scrapers.brokers.jll.scraper import JLLScraper
    
    # Create a custom scraper with our test components
    scraper = JLLScraper()
    scraper.client = MCPClient()
    scraper.storage = DataStorage("jll")
    
    logger.info("Running JLL scraper test")
    ...
    if results.get("success"):
        ...
        logger.info(f"Successfully extracted {len(properties)} properties from JLL")
...
async def main():
    """Run the test."""
    logger.info("Starting JLL scraper test")
    results = await test_jll_scraper()
    
    # Save results to a file
    with open("jll_test_results.json", "w") as f:
