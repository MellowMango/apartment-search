#!/usr/bin/env python
"""
Test script for MCP-based scraping of nmrk's multifamily properties.
This script demonstrates property listing extraction from nmrk's website.
"""
...
async def test_nmrk_scraper() -> Dict[str, Any]:
    """
    Test the nmrk scraper.
    
    Returns:
        Extracted property data
    """
    from backend.scrapers.brokers.nmrk.scraper import NmrkScraper
    
    # Create a custom scraper with our test components
    scraper = NmrkScraper()
    scraper.client = MCPClient()
    scraper.storage = DataStorage("nmrk")
    
    logger.info("Running nmrk scraper test")
    ...
    if results.get("success"):
        ...
        logger.info(f"Successfully extracted {len(properties)} properties from nmrk")
...
async def main():
    """Run the test."""
    logger.info("Starting nmrk scraper test")
    results = await test_nmrk_scraper()
    
    # Save results to a file
    with open("nmrk_test_results.json", "w") as f:
