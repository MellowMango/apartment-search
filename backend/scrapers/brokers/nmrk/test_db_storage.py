#!/usr/bin/env python
"""
Test script for database storage of nmrk properties.
...
from backend.scrapers.brokers.nmrk.test_nmrk_scraper import test_nmrk_scraper

async def test_database_storage():
    """
    Test storing nmrk property data in both Supabase and Neo4j databases.
    """
    logger.info("Testing database storage for nmrk properties")
    
    # First, extract property data using the existing test script
    logger.info("Extracting properties using the nmrk scraper")
    extracted_data = await test_nmrk_scraper()
    ...
    storage = ScraperDataStorage("nmrk", save_to_db=True)
