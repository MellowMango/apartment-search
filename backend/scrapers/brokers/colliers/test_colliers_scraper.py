#!/usr/bin/env python
"""
Test script for MCP-based scraping of Colliers' multifamily properties.
This script demonstrates property listing extraction from Colliers' website.
"""
...
async def test_colliers_scraper() -> Dict[str, Any]:
    """
    Test the Colliers scraper.
    
    Returns:
        Extracted property data
    """
    from backend.scrapers.brokers.colliers.scraper import ColliersScraper
    
    # Create a custom scraper with our test components
    scraper = ColliersScraper()
    scraper.client = MCPClient()
    scraper.storage = DataStorage("colliers")
    
    logger.info("Running Colliers scraper test")
    ...
    if results.get("success"):
        ...
        logger.info(f"Successfully extracted {len(properties)} properties from Colliers")
...
async def main():
    """Run the test."""
    logger.info("Starting Colliers scraper test")
    results = await test_colliers_scraper()
    
    # Save results to a file
    with open("colliers_test_results.json", "w") as f:
