"""
Specialized scraper for Colliers website.
This scraper extracts property listings from https://sales.colliers.com/
...
class ColliersScraper:
    """
    Specialized scraper for Colliers website, focusing on multifamily properties.
    """
    
    def __init__(self):
        """Initialize the Colliers scraper."""
        self.client = MCPClient()
        self.storage = ScraperDataStorage("colliers", save_to_db=True)
        self.base_url = "https://sales.colliers.com"
        self.properties_url = self.base_url
        
        # Add a URL for multifamily properties specifically
        self.multifamily_url = f"{self.properties_url}?q=multifamily"
    
    async def extract_properties(self) -> Dict[str, Any]:
        """
        Extract property listings from the Colliers website.
        ...
        try:
            ...
            title = "Colliers Investment Sales Properties"
    ...
    async def _navigate_to_page(self, page_num: int) -> bool:
        ...
        try:
            ...
            page_url = f"{self.multifamily_url}&page={page_num}"
            ...
            next_page_script = f"""
                try {{
                    ...
                    // If not found, try clicking "Next" button repeatedly
    ...
    async def _extract_properties_from_html(self, html: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            property_cards = soup.select('.property-card, .listing-card, .card, .property-item')
            
            if not property_cards:
                # Try more generic selectors
                property_cards = soup.select('.property, .listing, article.property, .property-listing')
            ...
            if property_cards:
                ...
                for card in property_cards:
                    try:
                        # Try to determine if this is a multifamily property
                        card_text = card.get_text().lower()
                        if "apartment" not in card_text and "multifamily" not in card_text and "family" not in card_text and "unit" not in card_text:
                            # Skip if likely not a multifamily property
                            continue
                        
                        # Get property title
                        title = "Unlisted Property"
                        title_elem = card.select_one('h2, h3, h4, .title, .property-title, .listing-title')
                        if title_elem:
                        ...
                        type_elem = card.select_one('.property-type, .type, .asset-class')
                        if type_elem:
                        ...
                        property_info = {
                            ...
                            "location": location,
                            "units": units,
                            "property_type": property_type or "Multifamily",
                            "price": price,
                            "sq_ft": sq_ft,
                            "status": "Available",
                            "image_url": image_url,
