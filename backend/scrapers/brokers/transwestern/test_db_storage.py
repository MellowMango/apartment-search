#!/usr/bin/env python
"""
Test script for database storage of transwestern properties.
...
from backend.scrapers.brokers.transwestern.test_transwestern_scraper import test_transwestern_scraper

async def test_database_storage():
    """
    Test storing transwestern property data in both Supabase and Neo4j databases.
    """
    logger.info("Testing database storage for transwestern properties")
    
    # First, extract property data using the existing test script
    logger.info("Extracting properties using the transwestern scraper")
    extracted_data = await test_transwestern_scraper()
