#!/usr/bin/env python
"""
Test script for database storage of marcusmillichap properties.
...
from backend.scrapers.brokers.marcusmillichap.test_marcusmillichap_scraper import test_marcusmillichap_scraper

async def test_database_storage():
    """
    Test storing marcusmillichap property data in both Supabase and Neo4j databases.
    """
    logger.info("Testing database storage for marcusmillichap properties")
    
    # First, extract property data using the existing test script
    logger.info("Extracting properties using the marcusmillichap scraper")
    extracted_data = await test_marcusmillichap_scraper()
    ...
    storage = ScraperDataStorage("marcusmillichap", save_to_db=True)
