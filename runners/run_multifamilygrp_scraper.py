#!/usr/bin/env python3
"""
Run the MultifamilyGroup scraper with database storage enabled.
"""
...
# Force database settings - these are public keys for this project
...
from backend.scrapers.brokers.multifamilygrp.scraper import MultifamilyGroupScraper
from backend.scrapers.core.storage import ScraperDataStorage

async def run_multifamilygrp_scraper_with_db():
    """Run the MultifamilyGroup scraper with database storage."""
    start_time = time.time()
    
    logger.info("=== MultifamilyGroup Scraper ===")
    logger.info("Starting MultifamilyGroup scraper")
    ...
    storage = ScraperDataStorage("multifamilygrp", save_to_db=True)
    scraper = MultifamilyGroupScraper(storage=storage, mcp_base_url="http://localhost:3001")
    ...
    try:
        ...
        logger.info("=== MultifamilyGroup Scraper Completed Successfully ===")
        return properties
    except Exception as e:
        logger.error(f"❌ MultifamilyGroup scraper failed: {e}")
        return None
...
if __name__ == "__main__":
    ...
    # Run the scraper
    asyncio.run(run_multifamilygrp_scraper_with_db()) 